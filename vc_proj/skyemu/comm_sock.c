...
int tcp_talk(char *remoteip, unsigned short remoteport, char *buf, int len, char *result,int need_close) {
	...
	if (tcpconnected==0) {
		...
		src_addr.sin_port=htons(FROM_PORT);
		src_addr.sin_addr.s_addr=INADDR_ANY;
		ret=bind(tcpsock,(struct sockaddr *)&src_addr,sizeof(struct sockaddr_in));
		if (ret==-1) {
			printf("Bind localport failed\n");
			exit(-1);		
			//return 0;
	...
	memset(tmpbuf,0,sizeof(tmpbuf)-1);
	ret=recv(tcpsock, tmpbuf, sizeof(tmpbuf)-1, 0);
...
//
// unified tcp communication, send buffer, and place retrived data into result.
int tcp_talk2(char *remoteip, unsigned short remoteport, char *buf, int len, char *result,int need_close) {
	int ret, ret2;
	struct sockaddr_in addr;
	WSADATA wsaDATA;
	char tmpbuf[BUF_SIZE];
...
//unified tcp communication, send buffer
int tcp_send(char *remoteip, unsigned short remoteport, char *buf, int len) {
	int ret;
	struct sockaddr_in addr;
	WSADATA wsaDATA;
