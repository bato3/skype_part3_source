// skyrecode.c : Defines the entry point for the console application.
//
...
extern int make_tcp_client_sess1_pkt4n(char *globalptr, char *pkt, int *pkt_len);
extern int make_tcp_client_sess1_pkt5n(_MIPD_ char *globalptr, char *pkt, int *pkt_len);
...
extern int process_tcp_client_sess1_pkt3n(_MIPD_ char *globalptr, char *resp, int resp_len);
extern int process_tcp_client_sess1_pkt4n(_MIPD_ char *globalptr, char *resp, int resp_len);
extern int process_tcp_client_sess1_pkt5n(_MIPD_ char *globalptr, char *resp, int resp_len);
...
;
	...
	global->session_setup_completed=0;
...
int main_skypeclient_tcpconnect(_MIPD_ char *globalptr){
	...
	retcode=process_tcp_client_sess1_pkt3n(_MIPP_ globalptr, resp, resp_len);
	if (retcode==-1) {
	...
	// pkt 4 (first pkt in chat sync)

	make_tcp_client_sess1_pkt4n(globalptr, (char *)pkt, &pkt_len);
	resp_len=tcp_talk(destip,destport,pkt,pkt_len,resp,0);
	if (resp_len==-1){
		if(DEBUG_LEVEL>=100) printf("socket comm error\n");
		//tcp1_talk_deinit();
	...
	retcode=process_tcp_client_sess1_pkt4n(_MIPP_ globalptr, resp, resp_len);
	if (retcode==-1) {
	...
	// pkt 5
	make_tcp_client_sess1_pkt5n(_MIPP_ globalptr, (char *)pkt, &pkt_len);
	resp_len=tcp_talk(destip,destport,pkt,pkt_len,resp,0);
	if (resp_len==-1){
		if(DEBUG_LEVEL>=100) printf("socket comm error\n");
		//tcp1_talk_deinit();
	...
	retcode=process_tcp_client_sess1_pkt5n(_MIPP_ globalptr, resp, resp_len);
	if (retcode==-1) {
