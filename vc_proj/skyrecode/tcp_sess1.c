...
extern int encode41_sess1pkt1_first(char *globalptr, char *buf, int buf_limit_len);


extern int encode41_sess2pkt_confirm(char *globalptr, char *buf, int buf_limit_len, int seq);
extern int encode41_sess2pkt4(char *globalptr, char *buf, int buf_limit_len);
extern int encode41_sess2pkt7(char *globalptr, char *buf, int buf_limit_len);
...
int make_tcp_client_sess1_pkt4n(char *globalptr, char *pkt, int *pkt_len){
	int len;
	int tmplen;
	int blkseq;
	...
	memset(buf1,0,sizeof(buf1));
  	buf1_len=encode41_sess1pkt1_first(globalptr, buf1, sizeof(buf1));
	if (buf1_len<0) {
...
unsigned int make_tcp_client_sess1_pkt5n(_MIPD_ char *globalptr, char *pkt, int *pkt_len){
	int len;
	int tmplen;
	int blkseq;
	...
	memset(buf2,0,sizeof(buf2));
  	buf2_len=encode41_sess2pkt_confirm(globalptr, buf2, sizeof(buf2), 0);
...
//////////////////////////////////////////////////////////////
	...
	memset(buf_newblk1,0,sizeof(buf_newblk1));
  	buf_newblk1_len=encode41_newblk2(globalptr, buf_newblk1, sizeof(buf_newblk1));
...
//////////////////////////////////////////////////////////////
	...
	memset(buf4,0,sizeof(buf4));
  	buf4_len=encode41_sess2pkt4(globalptr, buf4, sizeof(buf4));
	...
	memset(buf5,0,sizeof(buf5));
  	buf5_len=encode41_sess2pkt_confirm(globalptr, buf5, sizeof(buf5),1);
	show_memory(buf5, buf5_len, "sess2pkt5");
	...
	show_memory(buf6, buf6_len, "sess2pkt6");

	blkseq=0x08;
	buf6_len=process_aes_crypt(globalptr, buf6, buf6_len, 1, blkseq, 0);
	...
	show_memory(buf7, buf7_len, "sess2pkt7");

	blkseq=0x09;
	buf7_len=process_aes_crypt(globalptr, buf7, buf7_len, 1, blkseq, 0);
	...
	memset(buf8,0,sizeof(buf8));
  	buf8_len=encode41_sess2pkt_confirm(globalptr, buf8, sizeof(buf8),2);
	show_memory(buf8, buf8_len, "sess2pkt8");

	blkseq=0x0A;
	buf8_len=process_aes_crypt(globalptr, buf8, buf8_len, 1, blkseq, 0);
	...
	show_memory(buf9, buf9_len, "sess2pkt9");

	blkseq=0x0B;
	buf9_len=process_aes_crypt(globalptr, buf9, buf9_len, 1, blkseq, 0);
	...
	/////////////////////////////////////
	// first bytes correction, block s
	/////////////////////////////////////
	// calculate for 4 and 5 byte fixing
