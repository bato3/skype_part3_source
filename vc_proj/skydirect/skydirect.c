// skydirect.c : Defines the entry point for the console application.
//
...
extern int tcp1_talk(char *remoteip, unsigned short remoteport, char *buf, int len, char *result,int need_close);
extern int tcp1_talk_recv(char *result,int need_close);
...
int main_skypeclient_tcpconnect(_MIPD_ char *globalptr){
	...
	*/
	...
	resp_len=tcp1_talk_recv(resp,0);
	if (resp_len==-1){
		if(DEBUG_LEVEL>=100) printf("socket comm error\n");
		//tcp1_talk_deinit();
		return -21;
