...
int mysub_SessionManager_CMD_RECV_Process_00788E80();
int mysub_call_on_reply_check_possible_Flush_decode_pkt_header_decode_fail(uint, uint, uint, uint ,uint);
int unpack_7_bit_encoded_to_dword(uint var1,uint var2,uint var3);
int mysub_local_realloc_alloc_getlast_err_exception(unsigned int var1,unsigned int var2);
...
int mysub_no_call_00724060(unsigned int var1,unsigned int var2,unsigned int var3,unsigned int var4,unsigned int var5);
int mysub_call_eax__free();
int mysub_unpack_7_bit_encoded(unsigned int var1,unsigned int var2,unsigned int var3,unsigned int var4,unsigned int var5);
int unpack_7_bit_encoded_to_dword___2(unsigned int var1,unsigned int var2,unsigned int var3);
int mysub_local_alloc_memerr_exception(unsigned int var1);
int print_buffer(char *str,unsigned int size);
int print_buffer2(char *str,unsigned int size);

uint run_mysub_unpack_7_bit_encoded=0;

//int mygen_no_call_00927FF0();
int mygen_no_call_00927FF0(uint eax,uint ecx,uint edx,uint *eax11,uint *ecx11,uint *edx11);
...
//unsigned int buflen=0x49;
unsigned int buflen=sizeof(buf)-1;
*/

//
// try2
//inf2.txt
//unsigned int buflen=0x4a;
//
/*
static char buf[]=
...
//unsigned char heap_alloc_buf[0x280]; //640
char *heap_alloc_buf;
...
uint value_02c3ed70; //some tmp buf 0x14=20 byte long
uint value_02c3ed74; //
uint value_02c3ed78; //
uint value_02c3ed7c; //
uint value_02c3ed80; //
...
int main(int argc, char* argv[])
{
	...
	mysub_SessionManager_CMD_RECV_Process_00788E80();
...
int mysub_SessionManager_CMD_RECV_Process_00788E80(){
	...
	printf("ENTER mysub_SessionManager_CMD_RECV_Process_00788E80\n");
	...
	value_02c3f818=(unsigned int)buf;
	value_02c3f844=buflen;
	value_02c3ed68=0;
	value_02e09da0=0x02e09da0;   // session cmd ptr..
	value_02c3f7f8=0;
    value_02c3f83c=0;
...
do {
	...
	value_02e09da0=0x02e09da0;

	//ebp_14=value_02e09da0;
	esi=ebp_14;
	...
	if (edx!=eax) {
		printf("session cmd 0x43 i sess cmd ptr check.. err, jump on hzkuda2\n");
//
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		exit(1);
	...
	eax=mysub_call_on_reply_check_possible_Flush_decode_pkt_header_decode_fail(edx,ecx1,0,8,eax);
	...
	printf("left=0x%08X buflen=0x%08X\n",value_02c3f844, buflen);

	print_buffer("COMPLETED:",0x40);

	print_buffer2("STR_STRUCT:",0x20);
	...
	printf("LEAVE mysub_SessionManager_CMD_RECV_Process_00788E80\n");
...
int mysub_call_on_reply_check_possible_Flush_decode_pkt_header_decode_fail(uint var1,uint var2,uint var3,uint var4,uint var5){
	...
	printf("ENTER: mysub_call_on_reply_check_possible_Flush_decode_pkt_header_decode_fail\n");
	...
	ebp=value_02c3f818;//buf_ptr;

	edx=value_02c3f844;//bytes left, buflen;

	value_02c3ed70=0; //hz poka
...
//00723e13:
	...
	eax=unpack_7_bit_encoded_to_dword(value_02c3ed68,value_02c3f818,value_02c3f844);
	
    //printf("first decoded int, value_02c3ed68=%X\n",value_02c3ed68);
	printf("First INT:%X\n",value_02c3ed68);
...
//00723e35:
	...
	//--ebx=02c3f83c
	//ebx=var5;
	ebx=value_02c3f83c;//max buf len 0x04b000
	...
	value_02c3f7fc=0;//sdes budet ptr on alloc buf

	esp_18=value_02c3f7f8;
	esi=esp_18;
...
//jmp 00723e56:
	...
	esi=value_02c3f7fc;//esi ne menyaetsya v konce cikla, ono menyaetsya cherez counteri
	...
	if (ebp >= eax){
		...
		//push ecx  --size
		//push esi  -- ptr//value_02c3f7fc
		mysub_local_realloc_alloc_getlast_err_exception(esi,ecx);
...
//00723e8c:
	...
	//printf("eax=%X,ecx=%X, diff=%X, heap_alloc_buf=%X\n",eax,ecx,eax-ecx,value_02c3f7fc);
	copy_memory1(eax,ecx,0);
	...
	//push edx //alloc buf
	//printf("edx=%X,ecx=%X\n",edx,ecx);
	//edx,value_value_02c3ed70,0x14
	//copy_memory2(edx,ecx,0x14);
	copy_memory2(edx,value_02c3ed70,0x14);
	...
	//push ecx1 //ecx=alloc, alloc+0x14..//value_02c3f7fc.
	//push ebx // 02c3f83c//value_02c3f83c//max len buff
	//push eax // 08
	//push ecx // 02c3f844
	//push edx // 02c3f818
	eax=mysub_unpack_7_bit_encoded(edx,ecx,eax,ebx,ecx1);
	edx=value_02c3f818;
	ecx=value_02c3f844;
	...
	//ecx=4
	//ecx=4
	ecx=esp_14;   // kolvo ciklov voobshe iz value_02c3ed68, gde lejit perviy dekodirovanniy int
...
while (eax < ecx);
	...
	printf("LEAVE: mysub_call_on_reply_check_possible_Flush_decode_pkt_header_decode_fail\n");
...
int mysub_unpack_7_bit_encoded(unsigned int var1,unsigned int var2,unsigned int var3,unsigned int var4,unsigned int var5){
	...
	printf("ENTER mysub_unpack_7_bit_encoded\n");

	run_mysub_unpack_7_bit_encoded++;
	//push ebx

	
	printf("heap_alloc_buf=%X,var5=%X\n",heap_alloc_buf,var5);

	if (run_mysub_unpack_7_bit_encoded==2){
		//exit(1);
	};
	...
	edx_buf=(unsigned char *)value_02c3f818;	
	eax=edx_buf[0];
	printf("1:READ BYTE7b=%X\n",eax);
	...
	//push ebx //ebx=02c3f844 --sohranenniy counter
	//push edi //edi=02c3f818 --ptr on buf
	//push ecx //ecx=02e32eec --alloc_buf+4//value_02c3f7fc+4  // 2 int !
	//ecx=02e32f00 alloc_buf+18
	//ecx=02e32f10 alloc_buf+28
	//ecx=02e32f28 alloc_buf+40
	//unpack_7_bit_encoded_to_dword___2(ecx,edi,ebx)
	eax=unpack_7_bit_encoded_to_dword___2(ecx,edi,ebx);
	...
	//mov eax, [esi]  //esi=alloc=02e32ee8//value_02c3f7fc
	//eax=0
	//alloc+14 = 02e32efc 
	//eax=esi;
	//esi_buf=(unsigned char *)value_02c3f7fc;
	//memcpy(&eax,esi_buf,4);
	memcpy(&eax,(char *)esi,4); //dobivaem znachenie pervogo int

	print_buffer("2:",0x20);

	//printf("heap_alloc_buf=%X,value_02c3f7fc=%X\n",heap_alloc_buf,value_02c3f7fc);
	//exit(0);
	printf("IF: eax=%X,ebp=%X\n",eax,ebp);
	//exit(0);
	...
	if (eax !=ebp){
		...
		if (eax == 1){
		   ...
		   do {
			...
			edx_buf=(unsigned char *)edx;//value_02c3f818;
			eax=edx_buf[0];
			...
			mygen_no_call_00927FF0(eax,ecx,edx,&eax11,&ecx11,&edx11);
		...
		eax=unpack_7_bit_encoded_to_dword___2(ebp,edi,ebx);
		...
		//(novaya !!!)
		//push ecx//31
		eax=mysub_local_alloc_memerr_exception(ecx); //heap_alloc_struct alloc
		...
		//push edx //31
		//push ecx //ecx=02c3f3fb --curr buf ptr//value_02c3f818
		//push eax //eax=02e2d438 --alloc2//value_02e2d438;
		//copy_memory2(alloc2,pkt_buf+0x13,0x31);
		copy_memory2(eax,ecx,edx);
	...
	//esi=02e32f2c
	//value_02c3f7fc+8// == int 3
	esi=esi+8;
	//esi=value_02c3f7fc+8;
	...
	//unpack_7_bit_encoded_to_dword___2(ecx,edi,ebx)
	eax=unpack_7_bit_encoded_to_dword___2(esi,edi,ebx);
	...
	printf("LEAVE mysub_unpack_7_bit_encoded\n");
...
int mysub_local_alloc_memerr_exception(unsigned int var1){
	...
	printf("ENTER mysub_local_alloc_memerr_exception\n");
	...
	//retn
	printf("LEAVE mysub_local_alloc_memerr_exception\n");
...
int unpack_7_bit_encoded_to_dword___2(unsigned int var1,unsigned int var2,unsigned int var3){
	...
	printf("ENTER unpack_7_bit_encoded_to_dword___2\n");
...
//zahod 3 cikl 1
	do {
		...
		buf_edx=(unsigned char *)value_02c3f818;
		edx=buf_edx[0];
        printf("READ BYTE=%X\n",edx);
		//printf("edx=%X\n",edx);
		//exit(0);
	...
	printf("LEAVE unpack_7_bit_encoded_to_dword___2\n");
...
int mysub_call_eax__free(){
	...
	printf("ENTER mysub_call_eax__free()\n");
	...
	//ret
	printf("LEAVE mysub_call_eax__free()\n");
...
int mysub_no_call_00724060(unsigned int var1,unsigned int var2,unsigned int var3,unsigned int var4,unsigned int var5){
	...
	printf("ENTER mysub_no_call_00724060\n");
...
//  };  // 16 byte zabili 0-yami
	...
	printf("LEAVE mysub_no_call_00724060\n");
...
/*
*   Vydelenie pamyati
*   Inicializaciya ptr na struct, in value_02c3f7fc
*   sanity checks
*/
...
int mysub_local_realloc_alloc_getlast_err_exception(unsigned int var1,unsigned int var2){
	...
	printf("ENTER mysub_local_realloc_alloc_getlast_err_exception\n");
	...
	heap_alloc_buf=(char *)malloc(edx);
	eax=(unsigned int)heap_alloc_buf;
	...
	printf("LEAVE mysub_local_realloc_alloc_getlast_err_exception\n");
...
int unpack_7_bit_encoded_to_dword(uint var1,uint var2,uint var3){
	...
	do{
		...
		//mov [edi],ecx  --ecx=04
		value_02c3ed68=ecx; //sohranyaem sdes..
		edi=ecx;
