...
//tcp third packet
////////////////////////////////
int make_tcp_pkt3(u16 seqnum, u32 rnd, int last_recv_pkt_num, char *pkt, int *pkt_len) {
	int len;
	int i;
	...
	*/
	...
	skype_thing	mythings_tmp[] = {
		{0, 0x00, 0x2D, 0},
		{0, 0x05, 0x05, 0},
	};
	int mythings_tmp_len=2;

	u32 slots_num[] = { 0x2D,   0x055A, 0x006E, 0x3D,
						0x0523, 0x01C1, 0x04D5, 0x036D,
						0x07A7, 0x01AA, 0x0309, 0x0303,
						0x38,   0xAB,   0x07DD, 0x07F6 };
...
*/
	...
	//pkt 2 
	result_len=main_pack(mythings, mythings_len, result, sizeof(result)-1 );
	bufheader_len=first_bytes_header(seqnum, bufheader, sizeof(bufheader)-1, result, result_len);
	if (bufheader_len==-1) {
		return -1;
	};

	// add header and 42 to pkt buf
	...
	for(i=0;i<slots_num_size;i++){
		...
		mythings_tmp[0].m=slots_num[i];
		mythings_tmp[0].n=0;

		mythings_tmp[1].m=nodes_num;
		mythings_tmp[1].n=0;

		result_len=main_pack(mythings_tmp, mythings_tmp_len, result, sizeof(result)-1 );

		if (i==0){ seqnum-=2; };
		bufheader_len=first_bytes_header2(seqnum, bufheader, sizeof(bufheader)-1, result, result_len);
		if (bufheader_len==-1) {
			return -1;
		};
		if (i==0){ seqnum+=2; };
	...
	seqnum=0x05D8;
	bufheader_len=first_bytes_size(seqnum, bufheader, sizeof(bufheader)-1, pkt_tmp, pkt_tmp_len);
	if (bufheader_len==-1) {
		return -1;
	};
...
int process_tcp_pkt3(char *pkt, int pkt_len) {
