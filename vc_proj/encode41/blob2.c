...
int encode41_setup2pkt(char *buf, int buf_limit_len){
	...
	u8 remote_uic[]= "\x00\x00\x01\x04\x00\x00\x00\x01\x9A"
...
"\x6D\x7B\x32\x23\xC5\x67\x97\x57\x54\xD5\x5D\x89\x29\x28\x0C"
;


	u8 challenge_response[]=
"\x7D\xC1\xC3\x67\xC0\x36\x63\xD6\x12\x50"
...
;
	...
	blob.obj_index = 5;
    blob.obj_data = 0;
	blob.data_ptr = (int)remote_uic;
	blob.data_size = sizeof(remote_uic)-1;
	...
	blob.data_size = sizeof(challenge_response)-1;
	...
	blob.obj_index = 0x06;
    blob.obj_data = 0;
	blob.data_ptr = (int)aes_nonce;
	blob.data_size = sizeof(aes_nonce)-1;
