...
;
	...
	blob.data_size = sizeof(newblk)-1;
	...
	blob.obj_index = 4;
    blob.obj_data = 0;
	blob.data_ptr = (int)cred;
	blob.data_size = sizeof(cred)-1;
...
int encode41_sess3pkt7_recurs(char *buf, int buf_limit_len){
	...
	blob.obj_index = 0x12;
    blob.obj_data = 0;
	blob.data_ptr = (int)peers;
	blob.data_size = strlen(peers)+1;
