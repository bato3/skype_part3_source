...
void zzn3_init(_MIPD_ big p)
{ /* find cnr - cubic non-residue - and snr - sixth root of unity. p=1 mod 3 */
    int cnr;
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
    ...
    decr(_MIPP_ p,1,mr_mip->w1); /* p-1 */
    subdiv(_MIPP_ mr_mip->w1,6,mr_mip->w3); /* (p-1)/6 */

    cnr=2;
    forever
    {
        nres_powltr(_MIPP_ cnr,mr_mip->w3,mr_mip->sru);          /* sru = cnr^(p-1)/6 */
        nres_modmult(_MIPP_ mr_mip->sru,mr_mip->sru,mr_mip->w1); /* w1 = cnr^(p-1)/3 */
        nres_modmult(_MIPP_ mr_mip->w1,mr_mip->sru,mr_mip->w2);  /* w2 = cnr^(p-1)/2 */
        redc(_MIPP_ mr_mip->w1,mr_mip->w1);
        redc(_MIPP_ mr_mip->w1,mr_mip->w1);

        if (size(mr_mip->w1)!=1 && size(mr_mip->w2)!=1) break;
...
void zzn3_mul(_MIPD_ zzn3 *x,zzn3 *y,zzn3 *w)
{
    ...
    { /* Chung-Hasan SQR2 */
        ...
        nres_modsub(_MIPP_ mr_mip->w1,w->a,w->a);
        nres_premult(_MIPP_ mr_mip->w3,mr_mip->cnr,w->b);
        nres_modsub(_MIPP_ mr_mip->w4,w->b,w->b);
...
void zzn3_inv(_MIPD_ zzn3 *w)
{
    ...
    nres_modadd(_MIPP_ mr_mip->w2,mr_mip->w1,mr_mip->w3);
    ...
    nres_modadd(_MIPP_ mr_mip->w2,mr_mip->w1,mr_mip->w4);
