...
// udp search prepare //
////////////////////////
int make_udp_search_prepare_by_country(char *req_user, char *req_country, u16 seqnum, char *send_pkt, int *s_len) {
	u8 result[0x1000];
	int result_len;
	u8 header[0x100];
	int header_len=5;
...
// udp request Search //
////////////////////////
int make_udp_reqsearch_pkt1(char *ourip,char *destip, u16 seqnum, u32 rnd, char *req_user, char *req_country,
	...
	seqnum42=seqnum;
	ret=make_udp_search_prepare_by_country(req_user, req_country, seqnum42, (char *)&send_pkt+send_len, &slen);
	if (ret==-1){
...
int process_udp_reqsearch_pkt1(char *pkt,int pkt_len,char *ourip,char *destip,char *mainbuf, u32 *mainbuf_len) {
	RC4_context rc4;
	u32 newrnd;
	u32 targetip;
	u32 publicip;
