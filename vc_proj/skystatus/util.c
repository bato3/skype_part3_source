...
/*
	if (1){
		...
		//query cmd $x = x*8+2
		//reply on cmd $x = (x*8+2)+1

		header_len=encode_to_7bit(header, 31*8+2, 5);

		show_memory(header,header_len,"cmd:");
	};
*/
...
// with cmd encode
//
int first_bytes_header_cmd(u16 cmd, u16 seqnum, char *header, int header_len, char *buf, int buf_len){	
	int len;
	...
	theader_len=encode_to_7bit(theader, cmd*8+2, 5);
	if (theader_len==-1){
		return -1;
	};

	memcpy(header+len,theader,theader_len);
	len+=theader_len;
