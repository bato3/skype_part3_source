...
// tcp first packet //
//////////////////////
int make_tcp_client_sess1_pkt1_calc(_MIPD_ char *globalptr, char *pkt)
{
	...
	global->BLOB_1_9_ptr=0x48825033;
	global->BLOB_1_9_size=0x59A4F9F9;


	if(DEBUG_LEVEL>=100) printf("calced first TCP packet\n");
...
int process_tcp_client_sess1_pkt2(_MIPD_ char *globalptr, char *resp, int resp_len){
	...
	if (1){
		...
		if(DEBUG_LEVEL>=100) printf("before 41 parse\n");

		main_unpack_getdata1(data, datalen, cred_tmp, rnd64bit_tmp, &remote_session_id);
		

		show_memory(cred_tmp,0x188,"cred:");
		show_memory(rnd64bit_tmp,0x8,"rnd64bit:");
		
		if(DEBUG_LEVEL>=100) printf("remote_session_id: 0x%08X\n",remote_session_id);


		// 00 03 , m
		//global->REMOTE_SESSION_ID=remote_session_id;

		//global->REMOTE_SESSION_ID=global->connid;
		global->REMOTE_SESSION_ID=remote_session_id;
		...
		memcpy(rnd64bit, rnd64bit_tmp, 4);
		memcpy(rnd64bit+4, rnd64bit_tmp+4, 4);

		//print it
		show_memory(rnd64bit, 8, "rnd64bit");

		// 4 5 alloc , size188
		memcpy(global->REMOTE_CREDENTIALS, cred_tmp+8, 0x100);
