...
//
// Parse cred inputs argv1 type 2
//
int parse_input_line_type2(char *line, char *public_key, char *secret_key, char *str_skypename, char *user_cred) {
	u32	i;
	int len;
	u32 hex_digit;
	...
	sscanf(line,"%s %s %s %s %s %s %s %s %s %s",str_skypename,&str_password,&str_firstname,&str_lastname,&str_email,&str_version,
										  &str_cred,&str_p,&str_q,&str_format);
	...
	if ( strlen(str_p)<(0x80*2) ){
		if (DEBUG_LEVEL>=100) printf("pubkey parsing error, keys too small\n");
		return -1;
	};
	if ( strlen(str_q)<(0x80*2) ){
		if (DEBUG_LEVEL>=100) printf("seckey parsing error, keys too small\n");
		return -1;
	};
	...
	for (i = 0; i < 0x80; i++) {
		...
		hex_digit=convert_str_to_hex(p_ptr);
		public_key[i]=(char)hex_digit;
		p_ptr+=2;

		hex_digit=convert_str_to_hex(q_ptr);
		secret_key[i]=(char)hex_digit;
		q_ptr+=2;
