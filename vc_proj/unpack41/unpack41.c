// unpack41.c : Defines the entry point for the console application.
...
int mysub_SessionManager_CMD_RECV_Process_00788E80(char *buf1, uint buflen1, char *selfptr){
	...
	self->heap_main_alloc_struct_array[0]=NULL;
	self->heap_main_alloc_struct_array_size[0]=0;
	self->heap_main_alloc_struct_count=0;
	self->heap_main_current_alloc_struct_count=0;
...
do {
	...
	//self->heap_main_alloc_struct_array_size[self->heap_main_alloc_struct_count-1]=self->heap_alloc_buf_count ;//- 0x3c+0x14;
    self->heap_main_alloc_struct_array_size[0]=0x280;//self->heap_alloc_buf_count ;//- 0x3c+0x14;
...
int mysub_unpack_7_bit_encoded(uint var1,uint var2,uint var3,uint var4,uint var5, char *selfptr){
	...
	if (eax !=ebp){
		...
		if (eax == 0){
			...
			// ret eax = 1 if all ok
			if (DEBUG) { printf("perviy int=0, unpack return: eax=0x%08X\n",eax); };
		...
		if (eax == 5){
			//struct self_s self2;
			//struct self_s *self2ptr;
			//self2=(struct self_s *)self2ptr;
			//self2ptr=(char *)&self2;
				uint value_02c3f7f8;
				uint value_02c3f83c;
				uint value_02c3f7fc;
			...
			//exit(1);
			if (DEBUG) printf("RECURSION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");

			current_count = self->heap_main_current_alloc_struct_count;

			value_02c3f7f8 = self->value_02c3f7f8;
			value_02c3f83c = self->value_02c3f83c;
			value_02c3f7fc = self->value_02c3f7fc;
			value_02c3ed68 = self->value_02c3ed68;
			value_02e2d438 = self->value_02e2d438;

			if (DEBUG) {


				printf("before recursion start: current_count=%d\n",current_count);
				printf("healp_alloc_buf=0x%08X\n",self->heap_alloc_buf);
				printf("healp_alloc_buf_count=0x%08X\n",self->heap_alloc_buf_count);

				printf("value_02c3f7f8=0x%08X\n",self->value_02c3f7f8);
				printf("value_02c3f83c=0x%08X\n",self->value_02c3f83c);
				printf("value_02c3f7fc=0x%08X\n",self->value_02c3f7fc);
				printf("value_02c3ed68=0x%08X\n",self->value_02c3ed68);
				printf("value_02e2d438=0x%08X\n",self->value_02e2d438);
			...
			if (DEBUG) {
				printf("right after recursion end: heap_main_alloc_struct_count=%d\n",self->heap_main_alloc_struct_count);
				printf("right after recursion end: heap_main_current_alloc_struct_count=%d\n",self->heap_main_current_alloc_struct_count);
				printf("healp_alloc_buf=0x%08X\n",self->heap_alloc_buf);
				printf("healp_alloc_buf_count=0x%08X\n",self->heap_alloc_buf_count);
			};

			//self->heap_main_alloc_struct_array_size[self->heap_main_current_alloc_struct_count-1]=self->heap_alloc_buf_count;

			self->heap_main_current_alloc_struct_count=current_count;


			self->heap_alloc_buf=self->heap_main_alloc_struct_array[self->heap_main_current_alloc_struct_count-1];
			self->heap_alloc_buf_count=self->heap_main_alloc_struct_array_size[self->heap_main_current_alloc_struct_count-1];
			
			self->value_02c3f7f8 = value_02c3f7f8;
			self->value_02c3f83c = value_02c3f83c;
			self->value_02c3f7fc = value_02c3f7fc;
			self->value_02c3ed68 = value_02c3ed68;
			self->value_02e2d438 = value_02e2d438;

			if (DEBUG) {
				printf("after recursion end: current_count=%d\n",current_count);
				printf("healp_alloc_buf=0x%08X\n",self->heap_alloc_buf);
				printf("healp_alloc_buf_count=0x%08X\n",self->heap_alloc_buf_count);
			...
			if (DEBUG) printf("RECURSION END!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");


			//self->heap_main_alloc_struct_array_size[self->heap_main_alloc_struct_count-1]=self->heap_alloc_buf_count;

			//self->heap_main_alloc_struct_array[self->heap_main_alloc_struct_count]=heap_alloc_struct;
			//self->heap_main_alloc_struct_array_size[self->heap_main_alloc_struct_count]=edx;
			//self->heap_main_alloc_struct_count++;
			//self->heap_alloc_buf=heap_alloc_struct;

			//self->heap_alloc_buf=self->heap_main_alloc_struct_array[self->heap_main_alloc_struct_count-1];
			//self->heap_alloc_buf_count=self->heap_main_alloc_struct_array_size[self->heap_main_alloc_struct_count-1];

			//self2.value_02c3f818=(unsigned int)self->value_02c3f818;
			//self2.value_02c3f844=0x1000;
			//eax=mysub_call_on_reply_check_possible_Flush_decode_pkt_header_decode_fail(edi,ebx,ecx,edx,ebp,(char *)&self2);
...
///////////////////////////////////////////
		...
		//ne prigaem
		if (eax != 4){
			printf("perviy int != 4 (new unknown case), hz 29, eax=0x%08X\n",eax);
			exit(-1);
		};
		//eax==4
...
int mysub_local_alloc_memerr_exception(unsigned int var1, char *selfptr){
	...
	//save current main
	self->heap_alloc_struct_array_mainptr[self->heap_alloc_struct_count]=self->heap_main_current_alloc_struct_count;
...
int mysub_local_realloc_alloc_getlast_err_exception(unsigned int var1,unsigned int var2, char *selfptr){
	...
	if (DEBUG) printf("NEW HEAP ALLOC: 0x%08X\n",edx);

	if (DEBUG) printf("NEW HEAP ALLOC buf count: 0x%08X\n",self->heap_alloc_buf_count);

	self->heap_main_alloc_struct_array_size[self->heap_main_alloc_struct_count-1]=self->heap_alloc_buf_count - 0x14 + 0x14;
	...
	heap_alloc_struct=(char *)malloc(edx);
	//save ptr
	self->heap_main_alloc_struct_array[self->heap_main_alloc_struct_count]=heap_alloc_struct;
	//save size
	self->heap_main_alloc_struct_array_size[self->heap_main_alloc_struct_count]=edx;
