...
extern int main_unpack_once (FILE *fp, u8 *indata, u32 inlen);

int show_memory(FILE *fp, char *mem, int len, char *text);
int process_pkt(FILE *fp, char *pkt, int pkt_len, int use_replyto);
...
int show_memory(FILE *fp, char *mem, int len, char *text){
	...
	fprintf(stdout,"Len: 0x%08X\n",len);

	zz=0;
	for(i=0;i<len;i++){
		fprintf(fp,"%02X ",mem[i] & 0xff);
		fprintf(stdout,"%02X ",mem[i] & 0xff);
		zz++;
		if (zz == 16) { 
			zz=0; 
			fprintf(fp,"\n ");
...
int process_pkt(FILE *fp, char *pkt, int pkt_len, int use_replyto){
	...
	while(len > 0){
		...
		show_memory(fp, pkt+i+offset, 5, "pkt head at");

		current_pkt_size=get_packet_size(pkt+i+offset, 5, &current_pkt_size_len);
		current_pkt_size+=2;

		fprintf(fp,"current pkt size + 2(with header): 0x%X\n",current_pkt_size);
		fprintf(stdout,"current pkt size + 2(with header): 0x%X\n",current_pkt_size);
		fprintf(fp,"current pkt size length: %d\n",current_pkt_size_len);
		fprintf(stdout,"current pkt size length: %d\n",current_pkt_size_len);

		if (current_pkt_size > 0x1000){
			fprintf(fp,"pkt block size too big, len: 0x%08X\n",current_pkt_size_len);
			fprintf(stdout,"pkt block size too big, len: 0x%08X\n",current_pkt_size_len);
			//exit(1);
		...
		if (current_pkt_size <= 0){
			fprintf(fp,"pkt block size too small, len: 0x%08X\n",current_pkt_size_len);
			fprintf(stdout,"pkt block size too small, len: 0x%08X\n",current_pkt_size_len);
			exit(1);
		};
		...
		fprintf(fp,"recv_pktnum=0x%04X\n",bswap16(recv_pkt_num));
		fprintf(stdout,"recv_pktnum=0x%04X\n",bswap16(recv_pkt_num));
		offset+=2;
	   ...
	   if (offset < current_pkt_size) {
		...
		do {
			...
			fprintf(fp,"len_42_pkt: 0x%X\n",len_42_pkt);
			fprintf(stdout,"len_42_pkt: 0x%X\n",len_42_pkt);
			offset+=1;
			...
			if (len_42_pkt > 0x7f) {
				...
				show_memory(fp, pkt+i+offset-2, 5, "pkt 42 size head at");
				len_42_pkt=0;
			...
			fprintf(fp,"cmd len: %d\n",cmd_len);		
			fprintf(stdout,"cmd len: %d\n",cmd_len);		
			offset+=cmd_len;

			if ( (pkt[i+offset]!=0x42) && (use_replyto) ){
				reply_on_pkt=0;
				memcpy(&reply_on_pkt,pkt+i+offset,2);
				fprintf(fp,"reply_on_pkt=0x%04X\n",bswap16(reply_on_pkt));
				fprintf(stdout,"reply_on_pkt=0x%04X\n",bswap16(reply_on_pkt));
				offset+=2;
				len_42_pkt-=2;
			};

			if (pkt[i+offset]!=0x42){
				fprintf(fp,"next byte not 0x42 ! it is 0x%08X\n",pkt[i+offset] & 0xFF);
				fprintf(stdout,"next byte not 0x42 ! it is 0x%08X\n",pkt[i+offset] & 0xFF);
				exit(1);
			};

			len_42_processed=main_unpack_once(fp, pkt+i+offset,len_42_pkt);
			offset+=len_42_processed;

			if (len_42_processed != len_42_pkt){
				fprintf(fp,"len_42_pkt=0x%08X\n",len_42_pkt);
				fprintf(stdout,"len_42_pkt=0x%08X\n",len_42_pkt);
				fprintf(fp,"len_42_processed=0x%08X\n",len_42_processed);
				fprintf(stdout,"len_42_processed=0x%08X\n",len_42_processed);
				exit(1);
			};


			fprintf(fp,"offset at end = 0x%08X\n",offset);
			fprintf(stdout,"offset at end = 0x%08X\n",offset);
			fprintf(fp,"current pkt size = 0x%08X\n",current_pkt_size);
			fprintf(stdout,"current pkt size = 0x%08X\n",current_pkt_size);
		...
		fprintf(fp,"left = %d, bytes processed = %d\n",len,i);
		fprintf(stdout,"left = %d, bytes processed = %d\n",len,i);
